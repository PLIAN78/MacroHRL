...
for idx, name in regime_map.items():
    path_named = f'./models/sub_{name}.zip'
    path_idx   = f'./models/sub_{idx}_ppo.zip'
    import os
...
meta_model.save('./models/meta_e2e.zip')
...
# Save results
with open('./data/results_e2e.json', 'w') as f:
    json.dump(results, f, indent=2)
with open('./data/regime_dist_e2e.json', 'w') as f:
