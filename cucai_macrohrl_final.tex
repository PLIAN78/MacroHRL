...
\begin{tabular}{ccc}
\textbf{Neelesh Nayak} & \textbf{Peter Lian} & \textbf{Tony Xia} \\
...
r_k^p
-
c \sum_{i=1}^{N} |w_{k,i} - w_{k-1,i}|
-
\lambda \, \text{CVaR}_{\alpha}(L_k)
\end{equation}
...
\caption{MacroHRL framework architecture. Macro indicators are processed by a rule-based classifier to identify regimes. The Meta-Controller selects a specialized Sub-Controller, which executes daily allocation guided by a CVaR-penalized reward function for maximum drawdown control.}
...
We utilize daily price data for eight major ETFs (SPY, QQQ, EFA, EEM, TLT, HYG, GLD, VNQ) from 2010 to 2025. Macroeconomic indicators (VIX, CPI, Yield Curve) are sourced from FRED. The model is trained on 2010--2022 data and tested out-of-sample on 2023--2025.
